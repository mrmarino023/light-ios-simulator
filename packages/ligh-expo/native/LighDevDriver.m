...
static UIView *LighFindByLabel(UIView *root, NSString *label, int depth) {
    ...
    if (mine.length) {
        NSRange a = [mine rangeOfString:label options:NSCaseInsensitiveSearch];
        NSRange b = [label rangeOfString:mine options:NSCaseInsensitiveSearch];
        // Bidirectional: dump labels are often "Name, tab, 1 of 5" while the
        // live accessibilityLabel is just "Name" (or the reverse).
        if (a.location != NSNotFound || (mine.length >= 3 && b.location != NSNotFound)) {
            return root;
        }
    }
...
/// Activate a control the way VoiceOver / Switch Control would.
/// RN bottom-tabs and Pressables often ignore synthetic UITouch but honor this.
static BOOL LighActivate(UIView *view) {
    if (!view) return NO;
    UIView *target = LighPressableAncestor(view) ?: view;
    // Walk a short ancestor chain — tab items nest label → pressable → bar item.
    UIView *v = target;
    for (int i = 0; v && i < 8; i++, v = v.superview) {
        ...
        if (v.isAccessibilityElement || (v.accessibilityTraits & UIAccessibilityTraitButton) ||
            (v.accessibilityTraits & UIAccessibilityTraitTabBar) ||
            v.accessibilityLabel.length > 0) {
            if ([v accessibilityActivate]) return YES;
        }
        NSString *cls = NSStringFromClass(v.class);
        if ([cls containsString:@"Pressable"] || [cls containsString:@"Touchable"] ||
            [cls containsString:@"TabBar"] || [cls containsString:@"BottomTab"]) {
            if ([v accessibilityActivate]) return YES;
        }
    }
...
static NSDictionary *LighHandleOp(NSDictionary *op) {
    ...
    LighOnMain(^{
        ...
        } else if ([name isEqualToString:@"activate"] || [name isEqualToString:@"ax_activate"]) {
            // Explicit VoiceOver-style activate (best path for RN tabs/pressables).
            NSString *target = op[@"target"] ?: op[@"label"] ?: op[@"id"];
            UIView *v = nil;
            if (target.length) {
                v = LighFindByLabel(LighKeyWindow(), target, 0);
                if (!v) v = LighFindById(LighKeyWindow(), target, 0);
            } else if (op[@"nx"] && op[@"ny"]) {
                v = LighHitView(LighNormToPoint([op[@"nx"] doubleValue], [op[@"ny"] doubleValue]));
            }
            if (v && LighActivate(v)) {
                result = @{ @"ok": @YES, @"motor": @"accessibilityActivate" };
            } else {
                result = @{ @"ok": @NO, @"error": @"activate failed" };
            }
